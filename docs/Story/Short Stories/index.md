---
description: Short stories for ODODI
icon: material/book-open
---

# Short Stories

These stories are not locked to the main plot and serve to flesh out other elements of the world or simply random ideas I. Their canonicity is specified per page.

<div class="grid cards" markdown>

-   :material-book-open:{ .lg .middle } __Krath's Interview__

    ---

    Krath tries to abolish his debt and gets more than he bargained for.

    [:octicons-arrow-right-24: Krath's Interview - Canon Edition](Krath's%20Interview%20-%20Canon%20Edition.md)

</div>

<div class="grid cards" markdown>

-   :material-book-open:{ .lg .middle } __An idi0tic Entrance__

    ---

    Amirlithic soldier Keppelky gets scared by a big coob.

    [:octicons-arrow-right-24: An idi0tic Entrance](An%20idi0tic%20Entrance.md)

</div>


<div class="grid cards" markdown>

-   :material-book-open:{ .lg .middle } __The Great Inventor__

    ---

    Trauma-ridden weirdo attempts to bring back the dead at 3AM, gone wrong.

    [:octicons-arrow-right-24: The Great Inventor](The%20Great%20Inventor.md)

</div>